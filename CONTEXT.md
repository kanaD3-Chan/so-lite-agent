...
**Message tree（消息树）**:
会话内消息的**投影视图**（ADR-0007 转向后）：由会话事实日志经遮蔽投影派生——模型 history 是活跃投影链，人读 transcript 是全量日志；每条消息有 id 与 parentId，编辑 = 追加新 user 事件 + replace 遮蔽旧消息（"改完重发"语义），底层日志不可变、历史永不丢。**重新生成（改写 assistant）当前禁用**。
