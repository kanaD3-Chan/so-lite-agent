...
So Lite Agent（官方简写 **SL Agent**，引擎 crate 名 `so-lite-agent`）是**可执行文件项目**（ADR-0009，不发布 crates.io）：agent loop、工具注册与调度、会话生命周期、模型 Provider 抽象与通用 RPC 随源码提供；**内核插件与用户插件由使用方自行编写**（fork 定制者写 Rust 内核插件 crate / sl-agent 扩展者写 Rune 脚本）。Rust 2024 edition；仓库为 **Cargo workspace**（ADR-0008，P3 已落地）：`crates/engine`（引擎）+ `crates/plugin-*`（内核插件）+ `crates/sl-agent`（官方二进制），最终**编译成一个二进制** `sl-agent`。主交付是业务无关的通用 Agent API 服务（二进制 `sl-agent`，HTTP/WS；**前后端分离**——官方参考前端 = `frontend/` React 工程，ADR-0010）与 Rune 脚本用户插件路径，内核能力仅由维护者编译进官方二进制（Linus 模式，见 [docs/adr/0006](docs/adr/0006-pivot-harness-and-rune.md)）。mistake-agent 是本仓库的参考实现（保持独立二进制，见 [docs/adr/0001](docs/adr/0001-independent-repo-skip-m1.md)）。基于本项目开发自己的 Agent 见 [docs/agent-dev-guide.md](docs/agent-dev-guide.md)。
...
| 写 / 改插件 | docs/plugin-dev.md + crates/engine/examples/ | 两段式契约、enabled、目录编排、句柄注入 |
| 写 / 改内核 | docs/kernel-dev.md + crates/engine/src/agent/、crates/engine/src/registry/ | 模块地图、装配、调用链、扩展路径 |
| 接模型 Provider | docs/api.md §5 + crates/engine/src/model/ | ModelService、注册表、内置适配器 |
| 改 RPC / 事件 | docs/api.md §3-§4 + crates/engine/src/rpc.rs、crates/engine/src/events.rs | Method 子集、RpcExtension、Event::Custom |
...
- **通用运行时与业务分离**：错题、记忆、验算、settings 等业务领域类型与语义不进引擎 crate（ADR-0004）。仓库为 **Cargo workspace**（ADR-0008）：引擎 = `crates/engine`（`so-lite-agent`，业务无关）；内核插件以**独立 crate** 编写（`crates/plugin-*/`，自带依赖，不回流引擎）；官方二进制 = `crates/sl-agent`（依赖引擎 + 全部内置插件），整个 workspace 最终编译成**一个二进制**（`sl-agent`）；新增内核插件只改清单（根 members + sl-agent 依赖各一行），注册装配由 sl-agent 的 build.rs 自动发现（ADR-0036 改造）。
...
- 改动必须通过 `cargo test --all-targets --all-features` 与 `cargo clippy --all-targets --all-features -- -D warnings`（`--all-features` 覆盖 rune-plugins 门控代码）。
...
- docs 里的代码示例保持可编译/可运行（优先以 `crates/engine/examples/` 为准）。
